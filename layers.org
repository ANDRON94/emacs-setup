...
1. [[https://github.com/OmniSharp/omnisharp-roslyn][OmniSharp server]] (HTTP, STDIO) based on Roslyn workspaces.
...
(use-package unity
  :load-path ".cache/packages/unity/"
  :after (csharp-mode)
  :hook ((after-init . unity-build-code-shim)
         (after-init . unity-setup))
  :config
  (setq unity-var-directory (my-var "unity/")))
