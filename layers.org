...
#+begin_src emacs-lisp :tangle yes
(defun my--goto-end-of-eshell-prompt ()
  "Move cursor to the prompt while switching to evil insert mode (if evil is present)."
  (interactive)
  (goto-char (point-max))
  (when (featurep 'evil)
    (evil-append 1)))
...
(defun my--set-eshell-company-backends ()
  "Set the list of company backends for Eshell locally."
  (setq-local company-backends '((company-capf company-eshell-history))))

(defun my--set-evil-eshell-keybindings ()
  "Set custom evil eshell keybindings."
  (evil-define-key 'normal eshell-mode-map
    (kbd "<return>") #'my--goto-end-of-eshell-prompt))
...
(use-package eshell
  ...
  ;; Make `[return]' key more convenient for evil normal mode in eshell.
  (with-eval-after-load 'evil
    (add-hook 'eshell-mode-hook #'my--set-evil-eshell-keybindings))
  (with-eval-after-load 'company
    (add-hook 'eshell-mode-hook #'my--set-eshell-company-backends)))
#+end_src
** Version control
*** Git
